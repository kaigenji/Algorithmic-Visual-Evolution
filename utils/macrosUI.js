 ...
 * macrosUI.js
 *
 * Builds the UI for the "Macros & LFO" panel.
 * Each macro can have multiple parameter instances (for non‑color parameters, excluding cellSize).
 * For each parameter instance:
 *   - A "Parameter:" dropdown (fills horizontally)
 *   - A label "Influence (0–1):" with a slider (slider fills remaining space)
 *   - A Remove button below.
 *
 * Below the parameter instances:
 *   - An "Add Parameter" button that now only adds a parameter not already used in this macro.
 *   - LFO controls:
 *       - A row with "Trig Function:" + dropdown (fills horizontally)
 *       - A row with "Period (× tickRate):" + dropdown (fills horizontally)
 *       - A row with "Dampener (0–1):" + slider (slider fills horizontally)
 *   - A row with an "On/Off" toggle (checkbox)
 *   - Finally, a "Remove Macro" button at the bottom.
 */
...
function renderMacroContents(macro, macroDiv, onMacroChange) {
  ...
  macro.targets.forEach((tgt, index) => {
    ...
    influenceSlider.value = (tgt.influence || 1) * 100;
    influenceSlider.addEventListener('input', (e) => {
      tgt.influence = parseFloat(e.target.value) / 100;
      if (onMacroChange) onMacroChange(macro);
    });
  ...
  periodSelect.value = macro.periodMultiplier || 1.0;
  periodSelect.addEventListener('change', (e) => {
    macro.periodMultiplier = parseFloat(e.target.value);
    if (onMacroChange) onMacroChange(macro);
  });
  ...
  dampSlider.value = macro.dampener || 0.5;
  dampSlider.addEventListener('input', (e) => {
    macro.dampener = parseFloat(e.target.value);
    if (onMacroChange) onMacroChange(macro);
  });
  ...
  toggleCheckbox.addEventListener('change', (e) => {
    macro.enabled = e.target.checked;
    if (onMacroChange) onMacroChange(macro);
  });
