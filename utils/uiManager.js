...
function createRange(def, updateConfigFn) {
  ...
  slider.value = getConfigValueByPath(def.path);

  const valueDisplay = document.createElement('span');
  valueDisplay.textContent = Math.round(slider.value * 100) / 100;

  slider.addEventListener('input', (e) => {
    const val = parseFloat(e.target.value);
    valueDisplay.textContent = Math.round(val * 100) / 100;
...
function createColorSettings(updateConfigFn) {
  ...
  availableSchemes.forEach(opt => {
    const option = document.createElement('option');
    option.value = opt;
    option.textContent = opt.charAt(0).toUpperCase() + opt.slice(1);
    select.appendChild(option);
  });
  select.value = getConfigValueByPath("colorSettings.scheme");
  select.addEventListener('change', (e) => {
    const val = e.target.value;
    setConfigValueByPath("colorSettings.scheme", val);
    updateConfigFn("colorSettings.scheme", val);
  });
