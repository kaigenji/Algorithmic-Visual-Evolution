...
var require_regl = __commonJS({
  "node_modules/regl/dist/regl.js"(exports, module) {
    (function(global, factory) {
      ...
      function createExtensionCache(gl, config3) {
        var extensions = {};
        ...
        for (var i = 0; i < config3.extensions.length; ++i) {
          var name = config3.extensions[i];
          if (!tryLoadExtension(name)) {
            config3.onDestroy();
            config3.onDone('"' + name + '" extension is not supported by the current WebGL context, try upgrading your system or a different browser');
            return null;
          }
        }
        config3.optionalExtensions.forEach(tryLoadExtension);
        return {
          extensions,
          restore: function() {
      ...
      function wrapBufferState(gl, stats2, config3, destroyBuffer) {
        var bufferCount = 0;
        var bufferSet = {};
      ...
      function createTextureSet(gl, extensions, limits, reglPoll, contextState, stats2, config3) {
        var mipmapHint = {
        ...
        extend(REGLTexture.prototype, {
          bind: function() {
            ...
            if (unit < 0) {
              ...
              if (config3.profile && stats2.maxTextureUnits < unit + 1) {
      ...
      var wrapRenderbuffers = function(gl, extensions, limits, stats2, config3) {
        var formatTypes = {
          "rgba4": GL_RGBA4$1,
          "rgb565": GL_RGB565$1,
      ...
      function wrapShaderState(gl, stringStore, stats2, config3) {
        var fragShaders = {};
        var vertShaders = {};
      ...
      function reglCore(gl, stringStore, extensions, limits, bufferState, elementState, textureState, framebufferState, uniformState, attributeState, shaderState, drawState, contextState, timer, config3) {
      ...
      function wrapREGL(args) {
        ...
        var extensionState = createExtensionCache(gl, config3);
        if (!extensionState) {
          return null;
        }
        ...
        var shaderState = wrapShaderState(gl, stringStore, stats$$1, config3);
        ...
        var renderbufferState = wrapRenderbuffers(gl, extensions, limits, stats$$1, config3);
        ...
        var destroyCallbacks = [config3.onDestroy];
        var activeRAF = null;
        function handleRAF() {
...
function initializeReglApp() {
  ...
  canvas.width = config2.canvasWidth || window.innerWidth;
  canvas.height = config2.canvasHeight || window.innerHeight;
  const reglInstance = (0, import_regl.default)({
    canvas,
    extensions: ["oes_texture_float"]
    // Needed for float textures to store precise state
  ...
  let currentGrowthMultiplier = 1;
  let currentDecayMultiplier = 1;
  function updateEquilibrium(placeholderCoverage = 0.5) {
    const eqCfg = config2.equilibrium;
    const targetFraction = config2.activeCellDensity;
    const difference = targetFraction - placeholderCoverage;
    let relativeDiff = Math.log1p(Math.abs(difference));
    let growthScale = 1;
    let decayScale = 1;
    if (difference > 0) {
      growthScale = 1 + relativeDiff * eqCfg.maxExpansionScale * eqCfg.expansionResponse;
      decayScale = Math.max(1 - relativeDiff * eqCfg.decayResponse, eqCfg.minDecayScale);
    } else {
      growthScale = Math.max(1 - relativeDiff * eqCfg.expansionResponse, eqCfg.minExpansionScale);
      decayScale = 1 + relativeDiff * eqCfg.maxDecayScale * eqCfg.decayResponse;
    }
  ...
  const texWidth = Math.floor((config2.canvasWidth || window.innerWidth) / config2.cellSize);
  const texHeight = Math.floor((config2.canvasHeight || window.innerHeight) / config2.cellSize);
  console.log(`State texture dimensions: ${texWidth} x ${texHeight}`);
  const stateTextures = Array(2).fill(0).map(
    () => reglInstance.texture({
      width: texWidth,
      height: texHeight,
      type: "float",
      wrap: "clamp"
  ...
  const stateFbos = Array(2).fill(0).map(
    (_, i) => reglInstance.framebuffer({ color: stateTextures[i] })
  );
  let currentState = {
    readTex: stateTextures[0],
    writeTex: stateTextures[1],
    readFbo: stateFbos[0],
    // Keep FBOs for setting render target
  ...
  stateFbos.forEach((fbo) => {
    reglInstance({ framebuffer: fbo })(() => {
      reglInstance.clear({ color: [0, 0, 0, 0] });
    });
  });
  ...
  const initialCellData = new Float32Array([1, 0, 0, 1]);
  const initialTexture = currentState.readTex;
  initialTexture.subimage(
    { data: initialCellData, width: 1, height: 1 },
  ...
  const runUpdate = reglInstance({
    ...
    uniforms: {
      ...
      // Config uniforms
      neighborRadius: Math.floor(config2.creepingFig.neighborRadius),
      // Ensure integer
      branchAge: config2.creepingFig.branchAge,
      baseDecayChance: config2.decay.baseDecayChance,
      decaySlowdownFactor: config2.decay.slowdownFactor,
      minDecayChance: config2.decay.minDecayChance,
      edgeBreakChance: config2.decay.edgeBreakChance,
      // Binary transition uniforms
      binaryOn: config2.binaryTransitions.enabled,
      binaryThreshold: config2.binaryTransitions.threshold
    },
    framebuffer: () => currentState.writeFbo
  });
  const renderToScreen = reglInstance({
    frag: cellFragShader,
    // Use the final rendering shader
    vert: cellVertShader,
    attributes: {
      position: quadVertices
    },
    ...
    uniforms: {
      ...
      u_kaleidoscopeSectors: () => config2.kaleidoscope.sectors,
      u_kaleidoscopeCenter: () => [
        // Center needs to be normalized [0,1]
        config2.kaleidoscope.centerX / (config2.canvasWidth || window.innerWidth),
        config2.kaleidoscope.centerY / (config2.canvasHeight || window.innerHeight)
      ],
      u_kaleidoscopeAngleOffset: () => config2.kaleidoscope.angleOffset * (Math.PI / 180),
      // Convert degrees to radians
      u_kaleidoscopeZoom: () => config2.kaleidoscope.zoom,
      // --- Color Calculation Uniforms (from config) ---
      u_hueSpeed: () => config2.kaleidoscope.hueSpeed,
      u_saturation: () => config2.kaleidoscope.saturation,
      u_brightness: () => config2.kaleidoscope.brightness
    },
